    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const counts = {
                    bridalsarees: products.filter(product => product.category === 'bridalSaree').length,
                    bridesmaids: products.filter(product => product.category === 'bridesmaidSaree').length,
                    lehengas: products.filter(product => product.category === 'lehenga').length,
                    jwelleries: products.filter(product => product.category === 'jwellery').length,
                    bockeys: products.filter(product => product.category === 'flowerBouquet').length
                };
...
<div className="buttonlinkcontainer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop : "5%"}}>
  <Link to="/product/add" className={styles.buttonlink} style={{background:'#5B3E31', color:'#EDE1D2' }}>Upload Product</Link>
  
  <Link to="/product/generate-report" className={styles.buttonlink} style={{background:'#5B3E31', color:'#EDE1D2' }}>Generate Report</Link>

  <Link to="/product/all" className={styles.buttonlink} style={{background:'#5B3E31', color:'#EDE1D2' }}>Preview All Products</Link>
</div>
