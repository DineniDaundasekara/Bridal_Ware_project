...
import MSR from '../Img/SwarnaLogo.png';
...
const InventoryTable = () => {
    ...
    return (
        <div>
            <div className="header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 20px', backgroundColor: '#5B3E31'}}>
                <div className="logo" >
                ...
                <p style={{ color: "#D0BCA0", fontSize: "26px" }}> SWARNA </p>
            <p style={{ color: "black", fontSize: "32px", marginLeft: "7px" }}>
              {" "}
              BRIDALWARE{" "}
            </p>
                </div>
        ...
        <h1 style={{textAlign:'center', marginTop:'40px', marginBottom:'20px', textDecoration:'underline',background:'#D0BCA0', fontWeight:'675',color:'#81684A'}}>Current Stocks</h1>
        ...
        <img src={gif} style={{ marginBottom: '5px', width: '90%', height: '90%', marginTop: '-10px', cursor: 'pointer' }} alt="Description of GIF" />
          ...
          <h3 style={{ cursor: 'pointer',color:'#81684A'}} onClick={handleInstructionsClick}>Instructions</h3>
        </div>   
            ...
            <Link to="/inventory/addinventory" style={{ textDecoration: 'none' }}>
                ...
                {['Material', 'Button', 'Threads', 'Other'].map(type => (
                    <div key={type} style={{marginLeft:'10px', marginRight:'10px'}}>
                        <h2 style={{marginTop:'30px' , marginBottom:'25px' , marginLeft:'10px' , textDecoration:'underline' , textAlign:'center',color:'#81684A'}}>{type} Stocks</h2>
                ...
                {sortedInventoryData.map(item => (
                    item.raw_material_type === type && item.received_stock - item.used_stock > 0 && (
                        <tr key={item._id} style={{ backgroundColor: isLowStock(type, item.received_stock - item.used_stock) ? '#D0BCA0' : 'inherit' }}>
