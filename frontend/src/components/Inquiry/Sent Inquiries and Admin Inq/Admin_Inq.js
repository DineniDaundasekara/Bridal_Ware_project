...
async function handleReplySubmitAndSend() {
    ...
    return (
        <div className='bg' style={{backgroundColor: '#D0BCA0'}}>
            ...
            <h1 style={{ textAlign: "center", fontFamily: "sans-serif", textDecoration: "underline" , marginTop:'50px', marginBottom:'30px', fontWeight:'550',color:'#81684A' }}>All Inquiries</h1>
            <div style={{display:'flex', justifyContent:'center'}}>
            <p style={{marginTop:'32px', fontSize:'20px',color:'#562C1E',fontWeight:'bold'}}>Sort By</p>
            <div>
                <select value={searchType} className={styles.search0} onChange={handleSearchTypeChange} style={{background: '#8b6c5c',border:'#8F7664',color:'white'}}>
            ...
            <h2 style={{ textAlign: "center", fontFamily: "sans-serif", textDecoration: 'underline', fontSize: "30px", marginTop:'50px', marginBottom:'35px', fontWeight:'550',color:'#81684A' }}>Recent Inquiries</h2>

            {filteredPendingReplies.map(inquiry => (
                <div key={inquiry._id} className={styles.conatiner_all}>
                    {/* <button className={styles.Admin_buttons} onClick={() => handleReplyClick(inquiry._id, inquiry.Reply, inquiry.updatedAt)} style={{background:'#5B3E31',color:'#EDE1D2'}}>Reply</button> */}
                    <br /><br />
                    {/* <button className={styles.Admin_buttons} onClick={() => handleDeleteClick(inquiry._id)} style={{background:'#5B3E31',color:'#EDE1D2'}}>Delete</button> */}

                    <div className={styles.details}style={{marginTop:'5px'}}>
                        <h3 className={styles.marginBottom10} style={{ color: '#64473A', fontWeight: 'bold' }}>{inquiry.First_Name} {inquiry.Last_Name}</h3>
                        <p className={styles.marginBottom10} style={{fontWeight: 'bold' }}>Type: {inquiry.Type}</p>
                    {inquiry.Type === "Order Issue" && (
                        <p className={styles.marginBottom10} style={{fontWeight: 'bold' }}>Order ID: {inquiry.Order_ID}</p>
                    )}
                    {inquiry.Type === "Rent Issue" && (
                        <p className={styles.marginBottom10} style={{fontWeight: 'bold' }}>Rent ID: {inquiry.Rent_ID}</p>
                    )}
                    <p className={styles.marginBottom10} style={{fontWeight: 'bold' }}>Email: {inquiry.Email}</p>
                    <p className={styles.marginBottom10} style={{fontWeight: 'bold' }}>Contact Number: {inquiry.Contact_number}</p>
                    <p className={styles.marginBottom10} style={{fontWeight: 'bold' }}>Description: {inquiry.Description}</p>
                    <p className={styles.marginBottom10} style={{fontWeight: 'bold' }}>Date: {formatDate(inquiry.createdAt)}</p>
                    {inquiry.Reply && (
                    <div>
                    <h4 style={{ color: '#64473A', fontWeight: 'bold' }}>Reply:</h4>
                    <p style={{fontWeight: 'bold' }}>{inquiry.Reply}</p>
                    <div style={{ display: 'flex', justifyContent: '0px', width: '50%', marginLeft: '-400px' }}>
                    <button className={styles.Admin_buttons} onClick={() => handleReplyClick(inquiry._id, inquiry.Reply, inquiry.updatedAt)} style={{background:'#5B3E31',color:'#EDE1D2',textAlign:'center'}}>Reply</button>
                    <button className={styles.Admin_buttons} onClick={() => handleDeleteClick(inquiry._id)} style={{background:'#5B3E31',color:'#EDE1D2', marginLeft: '10px'}}>Delete</button>
                </div>
                </div>
            )}
        </div>
            ...
            <br /><br />
            <h2 style={{ textAlign: "center", fontFamily: "sans-serif", textDecoration: 'underline', fontSize: "30px" , marginTop:'50px', marginBottom:'25px', fontWeight:'550',color:'#81684A' }}>Submitted Replies</h2>
            <br />
            {filteredRepliesSent.map(inquiry => (
                <div key={inquiry._id} className={styles.conatiner_all}>

                    {/* <button className={styles.Admin_buttons} onClick={() => handleReplyClick(inquiry._id, inquiry.Reply, inquiry.updatedAt)} style={{background:'#5B3E31',color:'#EDE1D2', marginLeft: '10px'}}>Reply</button> */}
                    <br /><br />
